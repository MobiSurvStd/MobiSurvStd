...
def standardize(source: Path | ZipFile, skip_spatial: bool = False):
...
def read_files(source: Path | ZipFile, names: tuple[str, ...]):
    files = list()
    for name in names:
...
def persons_filename(source: Path | ZipFile) -> tuple[str, str, str] | None:
    return read_files(
