...
def standardize_trips(
    ...
    lf = lf.with_columns(
        ...
        destination_purpose_group=pl.col("Motif_D")
        .str.to_uppercase()
        .replace_strict(PURPOSE_GROUP_MAP),
        origin_insee=pl.when(pl.col("Code_INSEE_O").is_in(("ETRANGER", "HORS IDF")))
        .then(None)
        .otherwise("Code_INSEE_O"),
        destination_insee=pl.when(pl.col("Code_INSEE_D").is_in(("ETRANGER", "HORS IDF")))
        .then(None)
        .otherwise("Code_INSEE_D"),
        departure_time=pl.col("Heure_O").dt.hour().cast(pl.UInt16) * 60
        + pl.col("Heure_O").dt.minute(),
        arrival_time=pl.col("Heure_D").dt.hour().cast(pl.UInt16) * 60
        + pl.col("Heure_D").dt.minute(),
...
def standardize_distances(filename: str):
    ...
    lf = lf.with_columns(
        original_trip_id=pl.col("KEY"), trip_travel_distance_km=pl.col("Distance") * 1e-3
    )
