...
def clean(lf: pl.LazyFrame):
    ...
    if "age" in existing_cols:
        lf = add_age_class(lf)
    if "pcs_group_code" in existing_cols:
        lf = add_pcs_group(lf)
    if "workplace_singularity" in existing_cols:
        lf = add_work_only_at_home(lf)
...
def add_age_class(lf: pl.LazyFrame):
    lf = lf.with_columns(age_class=age_to_age_class(pl.col("age"))).with_columns(
        age_class_code=pl.col("age_class").replace_strict(
            AGE_CLASS_TO_CODE, return_dtype=PERSON_SCHEMA["age_class_code"]
        )
    )
    return lf
...
def add_pcs_group(lf: pl.LazyFrame):
    lf = lf.with_columns(
        pcs_group=pl.col("pcs_group_code").replace_strict(
            PCS_CODES, return_dtype=PERSON_SCHEMA["pcs_group"]
        )
    )
    return lf
...
def add_student_group(lf: pl.LazyFrame):
    lf = lf.with_columns(
        student_group=pl.col("student_category").replace_strict(
            STUDENT_GROUP_MAP, return_dtype=PERSON_SCHEMA["pcs_group"]
        )
    )
    return lf


def add_work_only_at_home(lf: pl.LazyFrame):
